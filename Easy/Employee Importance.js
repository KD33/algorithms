...
function solutionOne(employees, id) {
    const employeesOfInterest = [id];
    let importance = 0;

    while(employeesOfInterest.length > 0) {
        for(let i = 0; i < employees.length; i++) {
            const employee = employees[i];
            
            if (employee['id'] === employeesOfInterest[0]) {
                importance += employee['importance'];
                for (const sub of employee['subordinates']) {
                    employeesOfInterest.push(sub);
                }
                employeesOfInterest.shift();
